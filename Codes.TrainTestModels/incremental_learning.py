...
class BaseWeightBoosting(six.with_metaclass(ABCMeta, BaseEnsemble)):
    ...
    def __init__(self,
        ...
        self.all_ds_estimators_ = []
        self.all_ds_estimator_weights_ = np.empty(shape=(0))
    ...
    def decision_function(self, X):
        ...
        if self.algorithm == 'SAMME.R':
            ...
            pred = sum((estimator.predict(X) == classes).T * w
                       for estimator, w in zip(self.all_ds_estimators_,
                                               self.all_ds_estimator_weights_))
#             print "all_ds_estimators_", len(self.all_ds_estimators_)
#         else:   # self.algorithm == "SAMME"
#             pred = sum((estimator.predict(X) == classes).T * w
#                        for estimator, w in zip(self.estimators_,
#                                                self.estimator_weights_))

        pred /= self.all_ds_estimator_weights_.sum()
        if n_classes == 2:
            pred[:, 0] *= -1
